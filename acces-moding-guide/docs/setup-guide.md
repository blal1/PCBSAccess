...
If Tolk DLLs are missing, you must manually place them in the game directory. This is critical for screen reader communication.

1. **Download Tolk:** https://github.com/ndarilek/tolk/releases
2. **Identify Game Architecture:** Check Step 4 results (32-bit or 64-bit).
3. **Copy Files:**
   - **For 64-bit games (most modern games):**
     - Copy `Tolk.dll` from the `x64` folder.
     - Copy `nvdaControllerClient64.dll` from the `x64` folder.
   - **For 32-bit games (older games):**
     - Copy `Tolk.dll` from the `x86` folder.
     - Copy `nvdaControllerClient32.dll` from the `x86` folder.
4. **Placement:** Paste these files directly into the game's root directory (next to the main `.exe` file).

**IMPORTANT: You need BOTH DLLs!**
- `Tolk.dll` is the bridge for all screen readers.
- `nvdaControllerClient*.dll` is specifically required for **NVDA** support. Without it, NVDA will remain silent even if the mod loads correctly.

For beginners: Tolk is the translator that lets our mod "talk" to your screen reader. Think of `Tolk.dll` as the mouth and `nvdaControllerClient.dll` as the specific telephone line to NVDA.
