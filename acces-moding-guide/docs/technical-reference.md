...
The Handler pattern centralizes the logic for a specific game feature (e.g., Inventory, Main Menu) into a single class. This keeps the `Main` mod class clean and focused.
...
public class InventoryHandler
{
    ...
    // Static detection: Can be checked without an instance
    ...
    {
        _currentIndex = 0;
        ScreenReader.Say(Loc.Get("inventory_opened"));
    }

    private void Navigate(int direction)
    {
        // ... navigation logic ...
        ScreenReader.Say($"Item {_currentIndex + 1}");
    }
}
```
...
Only announce when values actually change to avoid spamming the user.
...
{
    // Cheap comparison every frame
    if (Mathf.Abs(currentHealth - _lastHealth) > 0.1f)
    {
        _lastHealth = currentHealth;
        // Only announce on actual change
        ScreenReader.Say($"Health: {currentHealth:F0}");
    }
}
```

### AccessStateManager (Exclusive Input)

When multiple handlers share the same keys, use `AccessStateManager` to manage focus.

```csharp
// In Main.cs
if (Input.GetKeyDown(KeyCode.Escape))
{
    // Try to let the active handler handle Escape first
    if (!AccessStateManager.HandleEscape())
    {
        // If no handler active, perform default game action (e.g., Pause)
    }
}
...
{
    AccessStateManager.OnEscapePressed += HandleEscape;
}

private void HandleEscape(AccessStateManager.State state)
{
    if (state == AccessStateManager.State.Inventory)
    {
        CloseInventory();
