...
- **Player Agency:** Provide the same information sighted players get, allowing the blind player to make their own strategic decisions.
...
To ensure maintainability and scalability, we use a modular architecture. This separates the logic for game detection, UI extraction, and accessibility output.

**Core Components:**
- **Main.cs:** The entry point. Handles mod initialization and distributes events to handlers.
- **ScreenReader.cs:** A clean wrapper around Tolk. All output should go through this class.
- **Loc.cs:** Central localization manager. All announced strings must be defined here.
- **Handlers:** Feature-specific classes (e.g., `InventoryHandler`, `MenuHandler`) that manage specific UI screens or game systems.
- **AccessStateManager:** (Optional) Manages exclusive input focus when multiple handlers share the same keys.

**Benefits of this Approach:**
- **Encapsulation:** Changes to the inventory UI only affect the `InventoryHandler`.
- **Readability:** The `Main` class remains small and easy to understand.
- **Consistency:** All screen reader output uses the same unified `ScreenReader.Say` method.
