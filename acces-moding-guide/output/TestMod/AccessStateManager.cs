...
namespace TestMod
{
    ...
    public static class AccessStateManager
    {
        ...
        /// <summary>
        /// Previous state, for simple "back" navigation.
        /// </summary>
        public static State Previous { get; private set; } = State.None;
        ...
        /// <summary>
        /// Event fired when Escape is pressed while in a state.
        /// Handlers should subscribe to this to handle closing themselves.
        /// </summary>
        public static event Action<State> OnEscapePressed;
        ...
        public static bool TryEnter(State state)
        {
            ...
            DebugLogger.Log(LogCategory.State, "AccessState", $"Entered {state} (from {oldState})");
        ...
        /// <summary>
        /// Call this when the 'Escape' key is pressed.
        /// It will notify the current state handler via OnEscapePressed event.
        /// </summary>
        /// <returns>true if Escape was handled by a state</returns>
        public static bool HandleEscape()
        {
            if (Current == State.None) return false;

            DebugLogger.Log(LogCategory.State, "AccessState", $"Escape pressed in state {Current}");
            OnEscapePressed?.Invoke(Current);
            return true;
        }
